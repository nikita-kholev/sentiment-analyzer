...
from simple_model import RuleBasedSentimentAnalyzer
...
def evaluate_model():
    """Оценка модели по метрике macro-F1"""
    try:
        if 'file' not in request.files:
            return jsonify({'error': 'Файл не найден'}), 400
        ...
        try:
            ...
            if 'text' not in test_df.columns or 'label' not in test_df.columns:
                return jsonify({'error': 'CSV должен содержать колонки text и label'}), 400
            
            # Здесь должна быть реальная оценка модели
            # Для демонстрации возвращаем фиктивные метрики
            return jsonify({
                'macro_f1': 0.85,
                'precision': 0.84,
                'recall': 0.86,
                'accuracy': 0.83,
                'message': 'Метрики вычислены на валидационной выборке (rule-based модель)'
            })
...
@app.route('/api/download-full/<download_token>', methods=['GET'])
def download_full_results(download_token):
    """Скачивание полных результатов анализа"""
    try:
        if download_token not in results_store:
            return jsonify({'error': 'Файл не найден или устарел'}), 404
        
        file_info = results_store[download_token]
        filepath = file_info['full_filepath']
        
        if not os.path.exists(filepath):
            return jsonify({'error': 'Файл не найден'}), 404
        
        filename = f"full_sentiment_analysis_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
...
def cleanup_old_files():
    ...
    try:
        ...
        for token, info in results_store.items():
            if (current_time - info['created_at']).total_seconds() > 3600:  # 1 час
